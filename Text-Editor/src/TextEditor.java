...
public class TextEditor extends JFrame implements ActionListener {
  ...
  public void Highlighter(JTextComponent txtComp, String pattern) {
    try {
      ...
      int pos = 0;
      while ((pos = textString.toUpperCase().indexOf(pattern.toUpperCase(),pos)) >= 0) {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() == findNextButton) {
      try {
        ...
        String textString = doc.getText(0, doc.getLength());
        if ((pos = textString.toUpperCase().indexOf(findTextField.getText().toUpperCase(), pos)) >= 0) {
          highlighter.addHighlight(pos, pos + findTextField.getText().length(), myHighlightPainter);
    ...
    if (e.getSource() == replaceItem) {
      ...
      JLabel label1 = new JLabel("replace with");
      txt.setSize(100, 50);
      txt2.setSize(100, 50);
    ...
    if (e.getSource() == goToButton) {
      try {
        Document doc = textArea.getDocument();
        String textString = doc.getText(0, doc.getLength());
        if ((pos1 = textString.toUpperCase().indexOf(goToField.getText().toUpperCase(),pos1)) >= 0) {
